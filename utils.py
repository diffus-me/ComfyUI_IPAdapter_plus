...
def get_lora_file(context: execution_context.ExecutionContext, pattern):
    lora_list = folder_paths.get_filename_list(context, "loras/ipadapter")
    lora_file = [e for e in lora_list if re.search(pattern, e, re.IGNORECASE)]
    lora_file = folder_paths.get_full_path(context, "loras/ipadapter", lora_file[0]) if lora_file else None
